    ...
    return (
        ...
        <div className="flex overflow-x-auto space-x-8 mt-8 no-scrollbar pb-8">
            ...
            {props.posts.data?.map((post: any, i: number) => (
                <Post key={post.id} id={post.id} title={post.attributes.title} subtitle={post.attributes.subtitle} tags={post.attributes.tags} date={post.attributes.date} url={post.attributes.url} type={post.attributes.type} content={post.attributes.content} image={post.attributes.image.data != null ? process.env.NEXT_PUBLIC_STRAPI_API + post.attributes.image.data.attributes.formats.medium.url : null } />
            ))}
        </div>
        </>
    );
