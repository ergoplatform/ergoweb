            ...
            <div className="relative">
                <Listbox.Button className="relative py-2 pl-3 pr-8 text-left cursor-default sm:text-sm">
                    ...
                    <span className="absolute inset-y-0 right-0 flex items-center pointer-events-none">
                        {color == "default" ? getIconComponentByName('LanguageSelector') : getIconComponentByName('LanguageSelectorWhite')}
                    </span>
                </Listbox.Button>
                <Transition
                    as={Fragment}
                ...
                >
                    <Listbox.Options className="absolute overflow-auto max-h-60">
                        {locales?.map((loc, key) => (
                            <Listbox.Option key={key} className={`cursor-default select-none relative`} value={loc} >
